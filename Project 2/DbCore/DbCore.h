...
* Maintenance History:
* --------------------
* ver 1.4 : 2 March 2018
* - edited show functions
* ver 1.3 : 17 Feb 2018
...
namespace NoSqlDb
{
 ...
 /* template<typename P>
  ...
  {
    out << "\n  ";
    out << std::setw(20) << std::left << el.name().substr(0, 13);
    out << std::setw(26) << std::left << std::string(el.dateTime());
    out << std::setw(20) << std::left << el.descrip().substr(0, 18);
    out << std::setw(25) << std::left << el.payLoad().path();
	out << std::setw(10) << std::left << el.payLoad().author();
	out << std::setw(10) << std::left << el.payLoad().status();
	out << std::setw(7) << std::left << std::to_string(el.payLoad().version());
	...
	std::vector<std::string> cat = el.payLoad().categories();
	if (cat.size() > 0)
	{
		out << "\n    Categories : ";
		for (auto key : cat)
  ...
  {
    out << "\n  ";
    out << std::setw(30) << std::left << key.substr(0,29);
    out << std::setw(15) << std::left << el.name().substr(0, 14);
    out << std::setw(26) << std::left << std::string(el.dateTime());
    out << std::setw(20) << std::left << el.descrip().substr(0, 18);
    out << std::setw(25) << std::left << el.payLoad().path().substr(0,24);
	out << std::setw(10) << std::left << el.payLoad().author();
	out << std::setw(10) << std::left << el.payLoad().status();
	out << std::setw(7) << std::left << std::to_string(el.payLoad().version());
	Children children = el.children();
